...
function App() {
  ...
  const [board, setBoard] = useState(new Array(31).fill('0000000000'))
  const [piece, setPiece] = useState(newPiece)
  const [baseBoard, setBaseBoard] = useState(new Array(31).fill('0000000000'))
  ...
  function reducer(state, action) {
    switch (action.type) {
      case 'down':
        return state.map(e => ({x: e.x, y: y+1}));
      case 'decrement':
  ...
  const [piece, dispatch] = useReducer(reducer, initialState);
